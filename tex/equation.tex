...
\begin{dmath}
 \cfrac {d} {d x}(x ^ {3}  + x ^ {2}  ) = 3 * x ^ {2}   + 2 * x ^ {1}   
\end{dmath}
...
Разложение в ряд тейлора с точностью до 3ого члена при x -> 0: 


\begin{dmath}
 x ^ {3}  + x ^ {2}   = x ^ {3}  + x ^ {2}   + (3 * x ^ {2}   + 2 * x ^ {1}   ) + (3 * 2 * x ^ {1}    + 2 * x ^ {0}   )
 + o(x^3)
\end{dmath}
